	...
	'test1' : function(TEST) {
		...
		var doTest = function(){
			...
			cache.getData('newval1').then(function(v){
				...
				TEST.equal(v,VALS.newval1,"default updater worked.");
				test_default_updater = true;
			});

			cache.getData('nonexistant').then(function(v){
				TEST.equal(v,undefined,"default updater worked - non-existant key.");
				test_default_updater_no_key = true;
			});

			cache.getData('something not here').then(function(v){
				TEST.equal(v,undefined,"default updater worked - non-existant key. (2)");
				test_default_updater_no_key_2 = true;
			});
 		...
 		// 			TEST.ok(r != GARBAGE,"post-invalidate, key was not 'garbage'");
			...
			setTimeout(function(){
				...
				console.log("ok - done.");
	     		...
	     		var test_post_clear_default_key = false;
	     		// post clear, try getting a key which the defaultUpdater can use
	     		cache.removeData('newval2').then(function(){
		     		cache.getData('newval2').then(function(v){
		     			SAY("GOT RESULT v=",v);
		     			TEST.equal(v,VALS.newval2,"default updater worked, after clear().");
		     			test_post_clear_default_key = true;
		     		});
	     		});
